...
import { ReportService, LISTING_REPORT_REASONS } from '../../app/lib/database/ReportService';
...
const ReportListingModal: React.FC<ReportListingModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!userId) {
      setError('You must be signed in to report listings');
      return;
    }
    ...
    try {
      const result = await ReportService.reportListing({
        listingId,
        reporterId: userId,
